...
// var dbURI = 'mongodb://sparrows:sparrows@ds045089.mongolab.com:45089/hotelsdb';
...
MongoClient.connect(dbURI, {
    ...
    if (!err) {
        db = _db;
        console.log("Connected to 'hotelsdb' database");
        _db.collection('hotels', {
            strict: true
        }, function(err, collection) {
            if (err) {
                console.log("The 'hotels' collection doesn't exist. Creating it with sample data...");
                // populateDB();
            }
        });
    } else {
...
exports.saveHotelRequest = function(req, res) {
    var hotelRequest = req.body;
    db.collection('HotelRequest', function(err, collection) {
        collection.insert(hotelRequest, {
            safe: true
        }, function(err, result) {
            if (err) {
                res.send({
...
exports.saveApiResponse = function(req, res) {
    var hotelApiResponse = req.body;
    console.log('API RESPONSE:'+hotelApiResponse);
    db.collection('HotelAPIResponse', function(err, collection) {
        collection.insert(hotelApiResponse, {
            safe: true
        }, function(err, result) {
            if (err) {
                res.send({
...
exports.trendingDestination = function(req, res) {
    db.collection('HotelRequest', function(err, collection){
        collection.aggregate([
            {$group : {_id : "$city", collection: {$push: "$$ROOT"}, count : {$sum : 1}}},
            {$sort: {count: -1}},
            {$limit: 10}
        ], function(err, result){
...
exports.findHotelByName = function(req, res) {
    var city = req.body.city;
    console.log('Retrieving hotelName: ' + city);
    db.collection('HotelAPIResponse', function(err, collection){
        collection.find({"HotelList": {$elemMatch: {city: city}}}).toArray(function(err, items) {
            res.send(items);
        });
    })
}
